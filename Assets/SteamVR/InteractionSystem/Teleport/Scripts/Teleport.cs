...
namespace Valve.VR.InteractionSystem
{
    ...
    public class Teleport : MonoBehaviour
    {
        ...
        public static List<(float x, float y, float z)> called_teleportation = new List<(float x, float y, float z)>();
        //-------------------------------------------------
        private void TeleportPlayer()
        {
            ...
            if (teleportingToMarker.ShouldMovePlayer())
            {
                ...
                // --- THIS IS THE VECTOR YOU WANT ---
                // Calculate the teleport target's position relative to the camera's local coordinate system.
                // This vector tells you where the target is relative to where the camera is currently looking.
                Vector3 relativePos = Camera.main.transform.InverseTransformPoint(teleportPosition);
                // --- VECTOR CALCULATION ENDS ---
                print(relativePos);

                // --- Logging/Storing Results ---
                // This line stores the X, Y, Z components of that relative vector:
                called_teleportation.Add((relativePos.x, relativePos.y, relativePos.z));
